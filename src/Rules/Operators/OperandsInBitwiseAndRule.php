...
use PhpParser\Node\Expr\BinaryOp\BitwiseAnd as BinaryBitwiseAnd;
 ...
 */
class OperandsInBitwiseAndRule implements Rule
{
	...
	{
		if ($node instanceof BinaryBitwiseAnd) {
			$left = $node->left;
			$right = $node->right;
		} elseif ($node instanceof AssignBitwiseOr) {
			$left = $node->var;
			$right = $node->expr;
		} else {
			return [];
